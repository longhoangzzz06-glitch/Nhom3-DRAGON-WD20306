        ...
        <div class="diary-entry <?= !empty($entry['incident']) ? 'incident' : '' ?>">
          ...
          <div class="diary-images">
            ...
            <img src="./uploads/tour_diary/<?= htmlspecialchars($image) ?>" 
                 alt="Tour image" 
                 class="diary-image"
                  onerror="this.onerror=null; this.src='./views/chung/img/no-image.png';"
...
function saveDiary(event) {
  ...
  .then(response => response.text()) // Luôn lấy RAW để debug
.then(raw => {
  console.log(raw); // Xem raw server trả về
  let data = null;
  try {
    data = JSON.parse(raw);
  } catch(e) {
  ...
  if (data.debug) console.log("PHP DEBUG:", data.debug);
